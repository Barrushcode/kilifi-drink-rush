...
const MpesaStkPush: React.FC<MpesaStkPushProps> = ({
  ...
  const isValidPhone = (value: string) => /^(\+254|0)[17]\d{8}$/.test(value);
  ...
  const initiateStkPush = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const customerName = shippingDetails ? `${shippingDetails.firstName} ${shippingDetails.lastName}` : "Customer";
  ...
  return <Card className="bg-barrush-charcoal/80 border-neon-pink border shadow-lg">
      <CardHeader>
      ...
      <CardContent>
        <form onSubmit={initiateStkPush} className="space-y-5">
          <div className="space-y-2">
            ...
            <Input id="mpesa-phone" type="tel" placeholder="e.g. 0712345678" value={userPhone} onChange={e => setUserPhone(e.target.value)} className="bg-neon-purple/40 border-neon-purple text-white placeholder:text-gray-400" required />
          </div>
          <div className="flex justify-between items-center text-white mb-2">
            <span className="text-lg">Total Amount:</span>
            <span className="text-xl font-bold text-neon-pink">KES {amount.toLocaleString()}</span>
          </div>
          <Button type="submit" disabled={processing || !userPhone} className={`w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-6 text-lg transition-all duration-300 ${processing && 'opacity-60 cursor-not-allowed'}`}>
