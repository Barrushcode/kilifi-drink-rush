...
const ProductsPagination: React.FC<ProductsPaginationProps> = ({
  ...
  return <>
          ...
          <PaginationContent>
            ...
            if (page === 1 || page === totalPages || page >= currentPage - 1 && page <= currentPage + 1) {
              return <PaginationItem key={page}>
                  ...
                  e.preventDefault();
                  setCurrentPage(page);
                }} isActive={isCurrentPage} className={`${isCurrentPage ? 'bg-rose-600 text-white border-rose-600' : 'bg-glass-effect border-barrush-steel/40 text-barrush-platinum hover:text-rose-600'} font-iphone h-touch min-w-touch`}>
                      {page}
                    </PaginationLink>
                  </PaginationItem>;
            }
      ...
      <div className="text-center mt-12 lg:mt-16">
        <div className="border rounded-xl p-6 lg:p-8 max-w-2xl mx-auto" style={{
        background: 'linear-gradient(135deg, rgba(255, 255, 255, 0.1) 0%, rgba(255, 255, 255, 0.05) 100%)',
        borderColor: 'rgba(55, 65, 81, 0.3)',
        backdropFilter: 'blur(4px)'
      }}>
          
          {filteredProductsLength > 0 && <p className="text-sm mt-2 font-iphone" style={{
